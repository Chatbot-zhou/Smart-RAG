...
if __name__ == '__main__':
    ss = StrategySelector()
    # [{'input':'我要处理一个意图识别的场景，选择bert微调作为模型更好，还是直接使用大模型'
    #  ,'label':'子查询检索'}]
    # strategy = ss.select_strategy('我要处理一个意图识别的场景，选择bert微调作为模型更好，还是直接使用大模型')
    # strategy = ss.select_strategy('番茄炒蛋怎么做')
    # strategy = ss.select_strategy('10万并发使用redis还是mysql')
    # strategy = ss.select_strategy('元芳，你怎么看？')
    strategy = ss.select_strategy('what can i say, man')
    print(strategy)
